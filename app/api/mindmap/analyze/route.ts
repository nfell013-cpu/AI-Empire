    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are MindMap, an AI mind mapping tool. Create comprehensive mind maps. Return JSON with: centralTopic, branches[{label, color, children[{label, children[], notes}]}], connections[{from, to, label}], summary, keyInsights[], suggestedResources[], actionableSteps[].`
          },
          {
            role: "user",
            content: input,
