  ...
  try {
    ...
    await prisma.tokenTransaction.create({
      data: {
        ...
        type: "usage",
        description: "PitchDeck analysis",
        toolSlug: "pitchdeck",
      },
    });
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are PitchDeck, an AI pitch deck generator. Create a compelling investor presentation. Return JSON with: title, subtitle, slides[{slideNumber, title, bullets[], speakerNotes, visualSuggestion}], financialProjections{year1, year2, year3}, competitorMatrix[], teamRoles[], askAmount, useOfFunds[], marketSize.`
          },
          {
            role: "user",
            content: input,
