...
import { Video, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";
...
export default function VideoSyncClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/videosync/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#dc262620" }}>
          <Video className="w-8 h-8" style={{ color: "#dc2626" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>VideoSync</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI video transcription & subtitle generator</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe your video concept, length, platform, and target audience..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Create Video Plan</>
          )}
        </button>
      </motion.div>
