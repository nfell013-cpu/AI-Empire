...
import { FileText, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function DocuWiseClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/docuwise/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>DocuWise</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>Intelligent document summarizer & key-point extractor</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Paste document text to summarize and extract key points..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Summarize Document</>
          )}
        </button>
      </motion.div>
