...
import { Shield, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function SecureNetClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/securenet/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#b91c1c20" }}>
          <Shield className="w-8 h-8" style={{ color: "#b91c1c" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>SecureNet</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI cybersecurity threat scanner & password auditor</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe your system, network, or application for security assessment..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Run Security Scan</>
          )}
        </button>
      </motion.div>
