...
import { FileText, DollarSign, Users, TrendingUp, Zap, ArrowRight, Lock, ShoppingBag, HardHat, Handshake, Leaf, PawPrint, Eye, Trophy, Globe, Search, Database, ShieldCheck, BarChart2, Music, MessageSquare, Coins } from "lucide-react";
...
export default function DashboardMetrics() {
  ...
  useEffect(() => {
    fetch("/api/tokens/balance").then(r => r.json()).then(d => { if (d.balance !== undefined) setTokenBalance(d.balance); }).catch(() => {});
  }, []);
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      ...
      {/* Token Balance & Earn Tokens CTA */}
      <motion.div
        initial={{ opacity: 0, scale: 0.98 }}
        animate={{ opacity: 1, scale: 1 }}
        className="rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4"
        style={{ background: "linear-gradient(135deg, rgba(234,179,8,0.15), rgba(239,68,68,0.1))", border: "1px solid rgba(234,179,8,0.3)" }}
      >
        <div className="flex items-center gap-4">
          <div className="p-3 rounded-xl" style={{ background: "rgba(234,179,8,0.2)" }}>
            <Coins className="w-8 h-8 text-yellow-400" />
          </div>
          <div>
            <p className="text-2xl font-bold text-yellow-400">{tokenBalance.toLocaleString()} Tokens</p>
            <p className="text-sm" style={{ color: "var(--text-secondary)" }}>Use tokens to access any AI tool. Watch ads or buy more.</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <EarnTokensButton compact onTokensEarned={() => fetch("/api/tokens/balance").then(r => r.json()).then(d => setTokenBalance(d.balance))} />
          <Link
            href="/earn-tokens"
