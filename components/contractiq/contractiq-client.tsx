...
import { Scale, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function ContractIQClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/contractiq/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#7c3aed20" }}>
          <Scale className="w-8 h-8" style={{ color: "#7c3aed" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>ContractIQ</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI contract generator & clause analyzer</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe the contract type and key terms, or paste existing contract text..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Generate Contract</>
          )}
        </button>
      </motion.div>
