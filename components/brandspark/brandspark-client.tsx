...
import { Sparkles, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function BrandSparkClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/brandspark/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#eab30820" }}>
          <Sparkles className="w-8 h-8" style={{ color: "#eab308" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>BrandSpark</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI brand name & logo concept generator</p>
        </div>
      </motion.div>
      ...
      >
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Generate Brand</>
          )}
        </button>
      </motion.div>
