...
import { ListChecks, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";
...
export default function TaskFlowClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/taskflow/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#2563eb20" }}>
          <ListChecks className="w-8 h-8" style={{ color: "#2563eb" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>TaskFlow</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI project management & task prioritization assistant</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe your project goals, team size, deadline, and constraints..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Create Project Plan</>
          )}
        </button>
      </motion.div>
