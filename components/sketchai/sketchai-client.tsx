...
import { PenTool, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function SketchAIClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/sketchai/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#d946ef20" }}>
          <PenTool className="w-8 h-8" style={{ color: "#d946ef" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>SketchAI</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>Sketch-to-design AI converter for UI/UX</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe the UI/UX design, wireframe, or sketch you need..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Generate Design</>
          )}
        </button>
      </motion.div>
