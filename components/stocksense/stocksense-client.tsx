...
import { TrendingUp, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function StockSenseClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/stocksense/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#10b98120" }}>
          <TrendingUp className="w-8 h-8" style={{ color: "#10b981" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>StockSense</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI stock & crypto market analysis with signals</p>
        </div>
      </motion.div>
      ...
      >
        <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-primary)" }}>
          Stock or market query
        </label>
        <textarea
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Enter stock ticker, market sector, or investment question..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Analyze Market</>
          )}
        </button>
      </motion.div>
