...
import { Mail, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";
...
export default function MailPilotClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/mailpilot/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#14b8a620" }}>
          <Mail className="w-8 h-8" style={{ color: "#14b8a6" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>MailPilot</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI email composer & response optimizer</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe the email you want to write or paste an email to optimize..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Compose Email</>
          )}
        </button>
      </motion.div>
