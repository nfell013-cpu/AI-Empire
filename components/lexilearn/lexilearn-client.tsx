...
export default function LexiLearnClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/lexilearn/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#a855f720" }}>
          <Cpu className="w-8 h-8" style={{ color: "#a855f7" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>LexiLearn</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI language tutor with conversation practice</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Enter the language and topic you want to learn (e.g., 'Spanish greetings')..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Start Lesson</>
          )}
        </button>
      </motion.div>
