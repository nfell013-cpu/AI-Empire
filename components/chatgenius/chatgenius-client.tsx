...
import { MessageSquare, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function ChatGeniusClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/chatgenius/analyze", {
        method: "POST",
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#8b5cf620" }}>
          <MessageSquare className="w-8 h-8" style={{ color: "#8b5cf6" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>ChatGenius</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>Custom AI chatbot builder for businesses</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe the business chatbot you want to build: industry, tone, key responses..."
        ...
        >
          {loading ? (
            ...
            <><Zap className="w-4 h-4" /> Build Chatbot</>
          )}
        </button>
      </motion.div>
