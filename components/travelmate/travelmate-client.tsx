...
import { Plane, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function TravelMateClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/travelmate/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#0891b220" }}>
          <Plane className="w-8 h-8" style={{ color: "#0891b2" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>TravelMate</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI flight & hotel deal finder with alerts</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe your destination, dates, budget, interests, and travel style..."
