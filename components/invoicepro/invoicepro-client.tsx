...
import { Receipt, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function InvoiceProClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/invoicepro/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#65a30d20" }}>
          <Receipt className="w-8 h-8" style={{ color: "#65a30d" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>InvoicePro</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI invoice generator & expense tracker</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Enter client name, services, amounts, payment terms..."
