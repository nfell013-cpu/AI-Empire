...
import { Mic, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function VoiceBoxClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/voicebox/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#f9731620" }}>
          <Mic className="w-8 h-8" style={{ color: "#f97316" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>VoiceBox</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI text-to-speech & voice cloning engine</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Enter text to convert to speech or describe the voice you want to clone..."
