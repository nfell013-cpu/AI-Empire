...
import { ImageIcon, Loader2, Copy, CheckCircle, Zap, AlertCircle } from "lucide-react";

export default function PixelCraftClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/pixelcraft/analyze", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="p-3 rounded-2xl" style={{ background: "#ec489920" }}>
          <ImageIcon className="w-8 h-8" style={{ color: "#ec4899" }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>PixelCraft</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>AI image generation & editing studio</p>
        </div>
      </motion.div>
